export const baseAPIURL = "https://edugator-admin.com/"; // For local testing with backend running, use "http://localhost:8080/. For actual use https://edugator-admin.com/"
export const unauthorizedErrorMessage =
  "You are not authorized to perform this action currently. Please log out and log back in.";

export const colors = {
  lightGray: "#f0f0f0",
  borderGray: "#e2e2e2",
  lightPink: "#fce4ec",
  redText: "#d81b60",
  navIconGray: "#969696",
};

export const adminPathRegex = /\/admin\/code*/;
